...
# -*- coding: utf-8 -*-
"""
train_xyz.py — Attention-BiLSTM training for metric XYZ gesture features.

Based on train_v2.py. Key differences for landmark_with_xyz.py output (175-dim):

Feature layout (175 channels):
  [0:72]    pose        — hand (21j×3) + arm (3j×3), metric XYZ, wrist-relative,
                          bone-length normalized (unitless)
  ...
  [173:174] hand_cov    — mean hand joint validity (scalar)
  [174:175] arm_cov     — mean arm joint validity (scalar)

Changes from train_v2.py:
  1. N_CONTINUOUS=148: separates continuous channels from binary validity channels
  2. gaussian_noise only applied to continuous channels [0:148]
  3. load_items reads subject_id directly from npz when available
  4. feature_block_mask and scale_jitter comments updated for XYZ layout
  5. Default save_path changed to runs/xyz
"""
...
# Number of continuous feature channels — validity channels follow after this index.
# XYZ layout: pose(72) + vel_pose(72) + vel_wrist(3) + scale(1) = 148
# Validity channels [148:175] are binary {0,1} and must not receive continuous augmentations.
N_CONTINUOUS = 148
...
# ================================================================
def parse_args():
    ap = argparse.ArgumentParser(description="Attention-BiLSTM trainer for XYZ features (LOSO / K-fold)")

    # Data
    ap.add_argument("--roots", nargs="+", required=True,
                    help="Directories with processed *.npz clips from landmark_with_xyz.py")
    ap.add_argument("--save_path", type=str, default="runs/xyz",
    ...
    ap.add_argument("--clip_norm", action="store_true", default=True,
                    help="Per-clip z-normalization on continuous channels only")
...
def extract_participant_id(path):
    """Fallback: extract participant ID from path like dataset/5/gesture_5_12.npz."""
...
def load_items(roots):
    ...
    for r in roots:
        ...
        for path in sorted(glob.glob(os.path.join(r, "*.npz"))):
            ...
            if vr < 0.25:
                continue
            # Prefer subject_id stored in the npz (landmark_with_xyz.py saves this)
            if "subject_id" in z:
                pid = str(int(z["subject_id"]))
            else:
...
def gaussian_noise(X, std=0.02):
    """Add Gaussian noise to continuous channels only — validity channels [N_CONTINUOUS:] are left intact."""
...
def feature_block_mask(X, prob=0.05):
    """Randomly zero entire continuous feature blocks.

    Feature layout for XYZ 175-dim (continuous portion, indices 0–147):
      pose(72) + vel_pose(72) + vel_wrist(3) + scale(1)
    Validity channels [148:175] are not touched.
    """
    if np.random.rand() > prob:
        return X
    X_out = X.copy()
    blocks = [
        (0, 72),       # pose (hand + arm XYZ, wrist-relative, bone-length normalized)
        (72, 144),     # velocity_pose
        (144, 147),    # velocity_wrist (metric XYZ, meters/frame)
        (147, 148),    # scale (metric bone-length, meters)
    ]
...
def scale_jitter(X, lo=0.8, hi=1.2):
    ...
    Simulates different body proportions / distances from camera.
    Only continuous pose+velocity channels are scaled [0:144].
    The metric scale channel [147] and validity channels [148:] are left intact.
    """
    X_out = X.copy()
...
class AugmentedClipSet(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # Per-clip z-normalization on continuous channels only — validity channels are binary
        if self.clip_norm:
...
def standardize_fit(train_items):
    ...
    # Only fit on continuous channels; validity channels are binary and should not be standardized
    Xc = Xtr[:, :, :N_CONTINUOUS]
    mu = Xc.mean(axis=(0, 1), keepdims=True).astype(np.float32)   # (1, 1, N_CONTINUOUS)
...
def main():
    ...
    if in_dim != 175:
        log.warning(f"Expected 175-dim XYZ features but got {in_dim}. "
                    f"Check that --roots points to landmark_with_xyz.py output.")
