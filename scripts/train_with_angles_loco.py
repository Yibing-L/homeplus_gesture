...
# -*- coding: utf-8 -*-
"""
train_with_angles_loco.py — Leave-One-Condition-Out (LOCO) variant.

Same Attention-BiLSTM and angle features as train_with_angles.py, but the
cross-validation loop holds out one *condition* (0-5) at a time instead of
one subject.  Labels are always 7-class (gesture identity, condition-
invariant) since the goal is to measure how well the model generalises
across experimental conditions.

Condition derivation: condition = y42 // 7   (6 conditions, indices 0-5)
Gesture label:        y7       = y42 % 7     (7 gestures)

Feature layout (207 channels) — identical to train_with_angles.py:
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Attention-BiLSTM trainer with joint angle features (LOCO / LOSO / K-fold)"
    )
...
def load_items(roots):
    ...
    for r in roots:
        ...
        for path in sorted(glob.glob(os.path.join(r, "*.npz"))):
            ...
            cond = y42 // 7   # condition index 0-5
            items.append(dict(path=path, X=X, valid=valid, y42=y42, cond=cond, pid=pid))
...
def standardize_apply(items, mu, sd, scale_mean=None):
    ...
    for it in items:
        ...
        out.append(dict(X=X.astype(np.float32), valid=it["valid"],
                        y42=it["y42"], cond=it.get("cond", -1),
                        pid=it.get("pid", "?"), path=it.get("path", "")))
    return out
...
def main():
    ...
    for fold_i, (train_items, val_items, test_items, fold_name) in enumerate(splits):
        ...
        for idx, it in enumerate(test_std):
            all_preds_with_pid.append(dict(pid=it["pid"], cond=it.get("cond", -1),
                                           y=y_test[idx], p=p_test[idx]))
    ...
    # Per-condition report (always useful for LOCO, also emitted for other modes if cond is present)
    all_conds = np.array([d["cond"] for d in all_preds_with_pid])
    unique_conds = sorted(set(all_conds))
    if len(unique_conds) > 1:
        clines = ["Condition  N_test  Accuracy  Macro-F1", "-" * 50]
        for cond in unique_conds:
            mask = all_conds == cond
            cy = np.array([d["y"] for d, m in zip(all_preds_with_pid, mask) if m])
            cp = np.array([d["p"] for d, m in zip(all_preds_with_pid, mask) if m])
            clines.append(f"{cond:>9d}  {len(cy):>6d}  "
                          f"{accuracy_score(cy, cp):>8.3f}  "
                          f"{macro_f1(cy, cp, n_classes):>8.3f}")
        cond_report = "\n".join(clines)
    ...
    if n_classes == 42 and args.eval_mode != "loco":
