...
# -*- coding: utf-8 -*-
"""
train_v1.py — Attention-BiLSTM training with LOSO CV and data augmentations.

Key improvements over train.py:
  - AttentionBiLSTM model with temporal attention pooling
  - Leave-one-subject-out (LOSO) cross-validation (default)
  - Data augmentation: speed perturbation, time warping, Gaussian noise,
    frame dropout, feature block masking
  - AdamW + OneCycleLR + gradient clipping
  - Label smoothing
  - Per-participant and per-condition evaluation reports
  - Confusion matrix PNGs and training curves
"""
...
def parse_args():
    ...
    ap.add_argument("--save_path", type=str, default="runs/v1",
    ...
    # Model
    ap.add_argument("--hidden", type=int, default=192, help="LSTM hidden size per direction")
    ap.add_argument("--lstm_layers", type=int, default=3, help="Number of BiLSTM layers")
...
class AugmentedClipSet(Dataset):
    def __init__(self, items, augment=False, aug_cfg=None):
        self.items = items
...
def generate_loso_splits(items):
    ...
    for test_pid in pids:
        ...
        rest = [it for it in items if it["pid"] != test_pid]
        # 10% of rest as val
        np.random.shuffle(rest)
        n_val = max(1, int(0.1 * len(rest)))
        val_items = rest[:n_val]
        train_items = rest[n_val:]
...
def main():
    ...
    for fold_i, (train_items, val_items, test_items, fold_name) in enumerate(splits):
        ...
        # Datasets
        ds_train = AugmentedClipSet(train_std, augment=(aug_cfg is not None), aug_cfg=aug_cfg)
        ds_val = AugmentedClipSet(val_std, augment=False)
        ds_test = AugmentedClipSet(test_std, augment=False)
