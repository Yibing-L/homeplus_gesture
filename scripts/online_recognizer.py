...
# -*- coding: utf-8 -*-
"""
online_recognizer.py — Live RGB–D gesture recognition for Home+.

This script mirrors the offline featurization (landmark.py) and the training pipeline (train.py)
so the model sees *exactly* the same input distribution at inference time.

Highlights
- MediaPipe Pose + Hands (TensorFlow disabled)
- RealSense RGB–D (aligned to color)
- Missed-frame handling identical to offline ("invalid" frame with zeros/NaNs)
- T_TARGET=64 resampling
- Valid-aware velocities (zeroed across invalid boundaries)
- Per-window velocity normalization
- Standardization with training mu/sd
- Valid-ratio gating and simple smoothing to stabilize outputs

Usage
------
python online_recognizer.py \
  --checkpoint bestcheckpoint.pt \
  --desired_hand Auto \
...
def parse_args():
    ...
    ap.add_argument("--checkpoint", type=str, required=True,
                    help="Path to neural checkpoint saved by train.py --save_path (PyTorch .pt only)")
...
# ---------------- Models (mirror train.py) ----------------
def build_model_from_artifact(artifact):
    mtype = artifact.get('type', None)
    if mtype not in ('tcn','gru','lstm','transformer'):
        raise ValueError(f"Unsupported model type: {mtype} (GBDT disabled)")
    ...
    try:
        while True:
            ...
            if vratio >= args.min_valid_ratio:
                ...
                with torch.no_grad():
                    # TCN ignores mask; others use it
                    logits = model(xt, mt) if model.__class__.__name__ != 'TCN' else model(xt)
